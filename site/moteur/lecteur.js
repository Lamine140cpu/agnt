...
/* Les deux séries montrent le même cadre ; seule leur DÉFINITION diffère. Le
   choix n'est donc plus une affaire de forme mais de nombre de pixels : sous
   820 px de large, la toile n'a jamais besoin de 1920, et servir la grande
   série y coûterait 41 Ko l'image au lieu de 23 — le double de la charge
   mobile pour un rendu identique. */
const etroit = (_cadre.width || innerWidth) < 820;
const NOM = (etroit && (!SOURCES || SOURCES['accueil-etroit']))
  ? 'accueil-etroit' : 'accueil';
const SEQ = SEQUENCES[NOM];
...
async function lire(i){
  try {
    const im = await decoder(await source(i));
...
const MEMOIRE_MAX = (innerWidth < 820 ? 144
...
let cibleY = scrollY, lisseY = scrollY;
...
if (molette) {
  ...
  addEventListener('scroll', () => {
    if (abs(scrollY - lisseY) > 2) { cibleY = scrollY; lisseY = scrollY; }
  }, { passive: true });
}
...
function boucle(now){
  ...
  /* 14 et non 9. Une fois le second rattrapage supprimé, il reste ce seul
     lissage : on peut donc le rendre plus franc sans que rien ne saute. Le
     temps de réponse passe de 263 à environ 170 ms, ce qui est le domaine où
     un geste et son effet se lisent comme simultanés. */
  if (molette && abs(cibleY - lisseY) > 0.4) {
  ...
  acteCourant();
  jauger(now, abs(vise - courant) > 0.05);
