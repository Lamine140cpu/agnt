...
let fetchData = true;
/* UNE REQUÊTE D'IMAGE DOIT POUVOIR MOURIR, et c'est ce qui manquait.
 *
 * Sans délai d'attente, une requête suspendue — le cas ordinaire d'une 4G qui
 * faiblit — ne se termine JAMAIS. Son numéro reste dans `encours` à vie, et
 * `fenetre()` ne lance rien tant que `encours.size < EN_VOL`. Les requêtes
 * mortes s'accumulent donc une à une : le budget de chargement se réduit, le
 * film avance de plus en plus mal, et à la vingt-quatrième le chargeur
 * s'arrête DÉFINITIVEMENT. La page ne rend plus la main.
 *
 * Signalé par un visiteur sur iPhone en 4G : « plus on avance plus ça ralentit,
 * puis c'est un freeze total ». Le symptôme décrit la panne exactement.
 *
 * Huit secondes : une image de trente kilo-octets qui n'est pas arrivée en huit
 * secondes n'arrivera pas à temps de toute façon — la fenêtre glissante est
 * passée à autre chose. Abandonner libère la place, et `lire` retire alors le
 * numéro de `encours`, ce qui rend l'image REDEMANDABLE au tour suivant.
 *
 * `AbortController` et non `AbortSignal.timeout` : ce dernier n'existe qu'à
 * partir de Safari 16, et ce lecteur porte déjà un repli pour Safari 14. */
const DELAI_IMAGE = 8000;
...
function source(i){
  ...
  ARRETS.set(i, arret);
  const minuteur = setTimeout(() => arret.abort(), DELAI_IMAGE);
  return fetch(`${SEQ.chemin}${String(i + 1).padStart(4, '0')}.jpg`,
               { signal: arret.signal })
    .then((r) => { if (!r.ok) throw new Error(r.status); return r.blob(); })
    .finally(() => { clearTimeout(minuteur); ARRETS.delete(i); });
}

/* LES REQUÊTES EN VOL, PAR NUMÉRO D'IMAGE. Elles servent à en abandonner une
   quand le défilement l'a dépassée — voir `elaguer`. */
...
/* ON ABANDONNE CE QUE LE DÉFILEMENT A DÉPASSÉ.
 *
 * Le délai d'attente seul ne suffit pas, et c'est mesuré : avec une requête sur
 * dix qui reste suspendue, huit secondes d'attente occupent en permanence une
 * vingtaine des vingt-quatre créneaux, et le film ne suit plus. Le chargeur
 * passe son temps à attendre des images dont personne ne veut plus.
 *
 * On coupe donc sur la PERTINENCE et non sur la montre : une image sortie de la
 * fenêtre glissante n'a plus aucune chance d'être affichée, quelle que soit sa
 * vitesse d'arrivée. L'abandonner rend son créneau tout de suite, et se règle
 * tout seul — plus on défile vite, plus on renonce tôt.
 *
 * La marge est celle de la fenêtre, doublée : on ne veut pas abandonner une
 * image que le visiteur rattraperait en remontant de quelques pixels. */
function elaguer(centre){
  if (!ARRETS.size) return;
  const marge = max(SEQ.avant, SEQ.arriere) * 2 + 8;
  for (const [i, arret] of ARRETS)
    if (abs(i - centre) > marge) arret.abort();
}
